clear all; clc
addpath(genpath('/export/mialab/users/eswar/software/export_fig'))
...
inname = '/export/mialab/users/eswar/fbirn_p3/results_C100_vn/FNCdynamics/FNC_dynamicSWTukey50_L1ICOV_zscored_scr_cnew';
...
FNCdynflat = Fdyn;
%%
matdet = zeros(Nwin,M);matdet_RSN = zeros(Nwin,M);
mnEf = zeros(Nwin,M); mnE = zeros(Nwin,M);
wcnt = 0;
for win =1:Nwin,
    wcnt = wcnt+1;
    scnt =0;
    for sub = 1:M, 
        scnt = scnt+1;subFdyn = vec2mat(squeeze(FNCdynflat(win,sub,:))) + eye(100);
        matdet(wcnt,scnt) = det(subFdyn);
        matdet_RSN(wcnt,scnt) = det(subFdyn(RSN_I,RSN_I));
        Ef = eig(subFdyn);             
        E = eig(subFdyn(RSN_I,RSN_I));
        mnEf(wcnt,scnt) = min(Ef);
        mnE(wcnt,scnt) = min(E);
    end;
    disp(['done window ' num2str(win) ' of ' num2str(Nwin)])
end
save minEig_GLASSO mnEf mnE
clear FNCdynflat
...
for ii = 1:Nwin
    rFdyn(:,ii,:) =  squeeze(Fdyn(:,ii,:)) - (mdynUNI.stats{1}.X(:, 3:10)*mdynUNI.stats{1}.B( 3:10,:));
end
[pth fnm ]= fileparts(inname);
save(fullfile(pth,[fnm '_residualized_1mancova_W50']),'rFdyn')
%%
[pth fnm ]= fileparts(inname);

% rFdyn [M, Nwin, C*(C-1)/2]
load(fullfile(pth,[fnm '_residualized_1mancova_W50']),'rFdyn')
...
% 
% save(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model_W50']), 'SPflat', 'IDXp','Cp');
 load(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model_W50']), 'SPflat', 'IDXp','Cp');
G = figure('Color', 'w', 'Name', ['FNC_group_clusters_k' num2str(k2) 'scrubbed_cnew_corr_resid1model_W50'], 'Position', CPOS);
for ii = 1:k2, 
    subplot(1,k2,ii);
...
%  load(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model']), 'SPflat', 'IDXp','Cp');
%  
%  [IDXp1, Cp1, SUMDp1, Dp1] = kmeans(SPflat1, k2, 'distance', dmethod, 'Replicates', 1, 'MaxIter', 150, 'Display', 'iter', 'empty', 'drop', 'Start', Cp);
%  
%  save(fullfile(fileparts(inname), ['FNC_half1_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model']), 'SPflat1', 'IDXp1','Cp1', 'sublist');
% %load(fullfile(fileparts(inname), ['FNC_half1_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model']), 'SPflat1', 'IDXp1','Cp1', 'sublist');
% 
% [IDXp2, Cp2, SUMDp2, Dp2] = kmeans(SPflat2, k2, 'distance', dmethod, 'Replicates', 1, 'MaxIter', 150, 'Display', 'iter', 'empty', 'drop', 'Start', Cp);
% 
% save(fullfile(fileparts(inname), ['FNC_half2_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model']), 'SPflat2', 'IDXp2','Cp2', 'sublist');
% %load(fullfile(fileparts(inname), ['FNC_half2_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model']), 'SPflat2', 'IDXp2','Cp2', 'sublist');
% 
% 
% G = figure('Color', 'w', 'Name', ['FNC_halfgroup_clusters_k' num2str(k2) 'scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
% for ii = 1:k2,
%     subplot(2,k2,ii);
...
% 
%     save(fullfile(fileparts(inname), ['FNC_HC_clusters_k2_' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'SPflatHC', 'IDXp1','Cp1', 'HCid');
%     
%     [IDXp2, Cp2, SUMDp2, Dp2] = kmeans(SPflatSZ, k2, 'distance', dmethod, 'Replicates', 1, 'MaxIter', 150, 'Display', 'iter', 'empty', 'drop', 'Start', Cp);
% 
%     save(fullfile(fileparts(inname), ['FNC_SZ_clusters_k2_' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'SPflatSZ', 'IDXp2','Cp2', 'SZid');
%     %load(fullfile(fileparts(inname), ['FNC_HC_clusters_k2_' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'SPflatHC', 'IDXp1','Cp1', 'HCid');
%     %load(fullfile(fileparts(inname), ['FNC_SZ_clusters_k2_' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'SPflatSZ', 'IDXp2','Cp2', 'SZid');
%     G = figure('Color', 'w', 'Name', ['FNC_DIAGgroup_clusters_k' num2str(k2) '_scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
%     for ii = 1:k2,
%         subplot(2,k2,ii);
...
%  %    aIND = aIND';
% %     
%    %  save(fullfile(fileparts(inname), ['FNC_clusters_all_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'IDXall', 'Call')
%    %  save(fullfile(fileparts(inname), ['FNC_all_states_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'aIND')
%     load(fullfile(fileparts(inname), ['FNC_clusters_all_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'IDXall', 'Call')
%     G = figure('Color', 'w', 'Name', ['FNC_clusters_all_k' num2str(k2) '_scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
%     for ii = 1:k2,
%         subplot(1,k2,ii);
...
%     timeline = 0:(Nwin-1); timeline = timeline + wsize/2; timeline = timeline*TR;
%     
%     H = figure('Color', 'w', 'Name', ['FNC_clusters_occurrence_k' num2str(k2) '_scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
%     for ii = 1:k2,
%         subplot(1,k2,ii);
%         for bb = 1:100
...
for k2 = 5%2:9
   ...
   % save(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model_W50']), 'IDXallone','aINDallone', 'Callone','SUMDallone','Dallone')
%     
    load(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model_W50']), 'IDXallone','aINDallone', 'Callone','SUMDallone','Dallone');
    aINDallone_SZ = aINDallone(SZid,:);
    aINDallone_HC = aINDallone(HCid,:);
    aINDHC = aINDallone_HC';
    aINDSZ = aINDallone_SZ';
    G = figure('Color', 'w', 'Name', ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model_W50'], 'Position', CPOS);
    for ii = 1:k2,
        subplot(2,k2,ii);
    ...
    H = figure('Color', 'w', 'Name', ['FNC_clusters_allone_occurrenceHCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model_W50'], 'Position', CPOS);
    for ii = 1:k2,
        subplot(2,k2,ii);
        for bb = 1:100
            pickME1 = ceil(nHC*rand(nHC,1));
...
save(fullfile(fileparts(inname), ['mn_mdn_FNC_clusters_alloneByDiag_kall_scrubbed_cnew_corr_resid1model_W50']), 'mean_Kwin_allHC', 'mean_Kwin_allSZ' , 'mdn_Kwin_allHC', 'mdn_Kwin_allSZ','nS_Kwin_allHC','nS_Kwin_allSZ');
%% stats
%load(fullfile(fileparts(inname), ['mn_mdn_FNC_clusters_alloneByDiag_kall_scrubbed_cnew_corr_resid1model']), 'mean_Kwin_allHC', 'mean_Kwin_allSZ' , 'mdn_Kwin_allHC', 'mdn_Kwin_allSZ','nS_Kwin_allHC','nS_Kwin_allSZ');
for k2 = 5%3:8
    mx = zeros(1,k2);mdx = mx;
    for jj = 1:k2
       [hh pp ci tst] = ttest2(mean_Kwin_allHC{k2-2,jj} ,mean_Kwin_allSZ{k2-2,jj});
    ...
    G1 = figure('Color', 'w', 'Name', ['FNC_clustersMNGroupDiff_alloneSubj_k' num2str(k2) '_scrubbed_cnewRev_corr_resid1model_W50'], 'Position', CPOSn);
    for jj = 1:k2
  ...
  %  close(G1)
    %%
    G2 = figure('Color', 'w', 'Name', ['FNC_clustersMDNGroupDiff_alloneSubj_k' num2str(k2) '_scrubbed_cnewRev_corr_resid1model_W50'], 'Position', CPOSn);
    for jj = 1:k2
...
%% plot tSNE and sammon distances between subject medians and state centroids
addpath('/export/mialab/users/eswar/software/Simple_tSNE_matlab'); 
for jj = 1:k2
    %%
    k5_allone_st_ALL_med = median(Fdynflat(IDXallone ==jj,:));
    k5_allone_st_ALL_mn = mean(Fdynflat(IDXallone ==jj,:));
    k5_allone_st_HC_med = median(FdynflatHC(aINDallone_HC(:)==jj,:));
    k5_allone_st_HC_mn = mean(FdynflatHC(aINDallone_HC(:)==jj,:));
    k5_allone_st_SZ_med = median(FdynflatSZ(aINDallone_SZ(:)==jj,:));
    k5_allone_st_SZ_mn = mean(FdynflatSZ(aINDallone_SZ(:)==jj,:));
    
%     figure;
%     subplot(2,3,1);imagesc(vec2mat(k5_allone_st_ALL_med),[-0.5 0.5]);axis square
%     subplot(2,3,2);imagesc(vec2mat(k5_allone_st_HC_med),[-0.5 0.5]);axis square
%     subplot(2,3,3);imagesc(vec2mat(k5_allone_st_SZ_med),[-0.5 0.5]);axis square
%     subplot(2,3,4);imagesc(vec2mat(k5_allone_st_ALL_mn),[-0.5 0.5]);axis square
%     subplot(2,3,5);imagesc(vec2mat(k5_allone_st_HC_mn),[-0.5 0.5]);axis square
%     subplot(2,3,6);imagesc(vec2mat(k5_allone_st_SZ_mn),[-0.5 0.5]);axis square
    
    k5_allone_st_HCsubj_med = mdn_Kwin_allHC{k2-2,jj};
    k5_allone_st_HCsubj_mn = mean_Kwin_allHC{k2-2,jj};
    k5_allone_st_SZsubj_med = mdn_Kwin_allSZ{k2-2,jj};
    k5_allone_st_SZsubj_mn = mean_Kwin_allSZ{k2-2,jj};

    allconcat = cat(1,k5_allone_st_ALL_med,k5_allone_st_ALL_mn,k5_allone_st_HC_med,k5_allone_st_HC_mn,k5_allone_st_SZ_med, ...
        k5_allone_st_SZ_mn,k5_allone_st_HCsubj_med,k5_allone_st_HCsubj_mn,k5_allone_st_SZsubj_med,k5_allone_st_SZsubj_mn);
    labels_allconcat = [1 2 3 4 5 6 7*ones(1,size(k5_allone_st_HCsubj_med,1)) 8*ones(1,size(k5_allone_st_HCsubj_med,1)) ... 
        9*ones(1,size(k5_allone_st_SZsubj_med,1)) 10*ones(1,size(k5_allone_st_SZsubj_med,1))]';
    rdist_allconcat = vec2mat(pdist(allconcat,'correlation'));
    edist_allconcat = vec2mat(pdist(allconcat,'euclidean'));
    %rP = sammon(rdist_allconcat,2);
    %eP = sammon(edist_allconcat,2);
    ydata_tSNEd_edist_allconcat = tsne_d(edist_allconcat, labels_allconcat,2);
    ydata_tSNEd_rdist_allconcat = tsne_d(rdist_allconcat, labels_allconcat,2);
    %%
    figure('color',[1 1 1],'name','corr dist: tSNE');hold on;
    for nn = 1:size(ydata_tSNEd_rdist_allconcat,1)
        if nn < 7
            mSiz = 12;
            if mod(nn,2) == 1 % median
                sym ='o';
            else % mean
                sym ='v';
            end
            ...
            mSiz = 8;
            if nn < 6+2*size(k5_allone_st_HCsubj_med,1) % HC
                if nn <= 6+size(k5_allone_st_HCsubj_med,1) % median
                    clr = 'k';sym = 'o';
                else % mean
                    clr = 'k';sym = 'v';
                end                   
                ...
                if nn < 6+2*size(k5_allone_st_HCsubj_med,1)+size(k5_allone_st_SZsubj_med,1) %median
                    clr = 'r';sym = 'o';
                else % mean
                    clr = 'r';sym = 'v';
                end
        ...
        plot(ydata_tSNEd_rdist_allconcat(nn,1),ydata_tSNEd_rdist_allconcat(nn,2),[clr sym],'MarkerSize',mSiz);
        
    end
    
    figure('color',[1 1 1],'name','euc dist: tSNE');hold on;
    for nn = 1:size(ydata_tSNEd_edist_allconcat,1)
        if nn < 7
            mSiz = 12;
            if mod(nn,2) == 1 % median
                sym ='o';
        ...
        plot(ydata_tSNEd_edist_allconcat(nn,1),ydata_tSNEd_edist_allconcat(nn,2),[clr sym],'MarkerSize',mSiz);
        
    end
end
%%
nCntHC = nS_Kwin_allHC{k2-2};nCntSZ = nS_Kwin_allSZ{k2-2};
aconcat = zeros(sum(nCntHC)+sum(nCntSZ)+k2*3,nCC); 
labels_aconcat = zeros(sum(nCntHC)+sum(nCntSZ)+k2*3,1);
ncnt = 0;
for jj = 1:k2
    %%
    k5_allone_st_ALL_med = median(Fdynflat(IDXallone ==jj,:));    
...
%     subplot(2,3,6);imagesc(vec2mat(k5_allone_st_SZ_mn),[-0.5 0.5]);axis square
    ...
    aconcat_st = cat(1,k5_allone_st_ALL_med,k5_allone_st_HC_med,k5_allone_st_SZ_med, ...
        k5_allone_st_HCsubj_med,k5_allone_st_SZsubj_med);
    labels_aconcat_st = jj*ones(nCntHC(jj)+nCntSZ(jj)+3,1);
    aconcat(ncnt+1:ncnt+nCntHC(jj)+nCntSZ(jj)+3,:) = aconcat_st;
    labels_aconcat(ncnt+1:ncnt+nCntHC(jj)+nCntSZ(jj)+3) = labels_aconcat_st;
    ncnt = ncnt+nCntHC(jj)+nCntSZ(jj)+3;
   
end
 rdist_aconcat = vec2mat(pdist(aconcat,'correlation'));
 edist_aconcat = vec2mat(pdist(aconcat,'euclidean'));
 %rP = sammon(rdist_aconcat,2);
 %eP = sammon(edist_aconcat,2);
 ydata_tSNEd_edist_aconcat = tsne_d(edist_aconcat, labels_aconcat,2);
 ydata_tSNEd_rdist_aconcat = tsne_d(rdist_aconcat, labels_aconcat,2);
 
save tSNE_subjmediandata_k5_W50 aconcat labels_aconcat ydata_tSNEd_edist_aconcat ydata_tSNEd_rdist_aconcat rdist_aconcat edist_aconcat
%%
FPOS = [635    66   965   849];
clr5 = 'rgbck';
FF0 = figure('color',[1 1 1],'position',FPOS,'name','euc dist tSNE Subj Medians all k5');hold on;
ncnt = 0;
for jj = 1:k2%size(ydata_tSNEd_edist_aconcat,1)
    clr = clr5(jj);
    for nn = 1:nCntHC(jj)+nCntSZ(jj)+3
        if nn < 4 % group medians            
            mSiz = 14;
            ...
            plot(ydata_tSNEd_edist_aconcat(ncnt+nn,1),ydata_tSNEd_edist_aconcat(ncnt+nn,2),[clr sym],'MarkerSize',mSiz,'LineWidth',2);
        else
            mSiz = 6;
            
            if nn <= 3+nCntHC(jj) % HC
            ...
            plot(ydata_tSNEd_edist_aconcat(ncnt+nn,1),ydata_tSNEd_edist_aconcat(ncnt+nn,2),[clr sym],'MarkerSize',mSiz);
        end
...
%export_fig('tSNE_allk5_subjMedians_eucdist.pdf',FF0)
%%
FF=figure('color',[1 1 1],'position',FPOS,'name','corr dist tSNE Subj Medians all k5');hold on;
ncnt = 0;
for jj = 1:k2%size(ydata_tSNEd_edist_aconcat,1)
    clr = clr5(jj);
    for nn = 1:nCntHC(jj)+nCntSZ(jj)+3
        if nn < 4 % group medians            
            ...
            plot(ydata_tSNEd_rdist_aconcat(ncnt+nn,1),ydata_tSNEd_rdist_aconcat(ncnt+nn,2),[clr sym],'MarkerSize',mSiz,'LineWidth',2);
            ...
            plot(ydata_tSNEd_rdist_aconcat(ncnt+nn,1),ydata_tSNEd_rdist_aconcat(ncnt+nn,2),[clr sym],'MarkerSize',mSiz);
...
%export_fig('tSNE_allk5_subjMedians_corrdist.pdf',FF)
...
save(fullfile(fileparts(inname),  'dfnc_SC_SM_relationships_k5_corr_recid1model'),  'r_SC*', 'r_THAL*', 'r_wSC*', 'p_SC*', 'p_THAL*', 'p_wSC*', '*allstates')
%%
load(fullfile(fileparts(inname), 'mn_mdn_FNC_clusters_alloneByDiag_kall_scrubbed_cnew_corr_resid1model'))
X1_HC_SC2SMcorr_recid1corr_boot = zeros(nBOOT,k2);
X1_HC_SCnoT2SMcorr_recid1corr_boot = zeros(nBOOT,k2);
...
for bb = 1:nBOOT,
    ...
    X1_HC_SCnoT2SMcorr_recid1corr_boot(bb,:) = cellfun(@mean,scnot_to_sm_corr_HC_allstatesb);
...
save(fullfile(fileparts(inname),  'dfnc_SC_SM_relationships_k5_corr_recid1model_boot'),'*_boot')
%%
load(fullfile(fileparts(inname),  'dfnc_SC_SM_relationships_k5_corr_recid1model'))
load(fullfile(fileparts(inname),  'dfnc_SC_SM_relationships_k5_corr_recid1model_boot'))
...
set(gca,'FontSize',14)
export_fig(fullfile(FIGoutputdir,'thal2SMgroupdiff_vs_scsmantagonism.pdf'),'-pdf')
...
ylabel('SZ - HC: mean thalamus to sensory FC')
set(gca,'FontSize',14)
export_fig(fullfile(FIGoutputdir,'thal2SMgroupdiff_vs_withinSM-FC.pdf'),'-pdf')
...
set(gca,'FontSize',14)
export_fig(fullfile(FIGoutputdir,'pput2SMgroupdiff_vs_scsmantagonism.pdf'),'-pdf')
...
ylabel('SZ - HC: mean putamen to sensory FC')
set(gca,'FontSize',14)
export_fig(fullfile(FIGoutputdir,'pput2SMgroupdiff_vs_withinSM-FC.pdf'),'-pdf')
%------
for kk = 1:5,
    figure;hist((Y_SZ_thal2SMcorr_recid1corr_boot(:,kk) - Y_HC_thal2SMcorr_recid1corr_boot(:,kk)),50);hold;plot(Y_SZ_thal2SMcorr_recid1corr(kk)-Y_HC_thal2SMcorr_recid1corr(kk),100,'r*')
end
for kk = 1:5,
    figure;
    subplot(1,2,1);hist(Y_SZ_thal2SMcorr_recid1corr_boot(:,kk),50);hold;plot(Y_SZ_thal2SMcorr_recid1corr(kk),100,'r*')
    subplot(1,2,2);hist(Y_HC_thal2SMcorr_recid1corr_boot(:,kk),50);hold;plot(Y_HC_thal2SMcorr_recid1corr(kk),100,'r*')
end
%-------
...
set(gca,'FontSize',12)
set(FF,'color',[1 1 1]);
export_fig(fullfile(FIGoutputdir,'dFNC_thalamus2SM-subcortex2SM-relationships.pdf'),'-pdf')
...
set(gca,'FontSize',12)
set(FF1,'color',[1 1 1]);
export_fig(fullfile(FIGoutputdir,'dFNC_thalamus2SM-wSensory-relationships.pdf'),'-pdf')
...
set(gca,'FontSize',12)
set(FF2,'color',[1 1 1]);
export_fig(fullfile(FIGoutputdir,'dFNC_putamen2SM-subcortex2SM-relationships.pdf'),'-pdf')
...
set(gca,'FontSize',12)
set(FF3,'color',[1 1 1]);
export_fig(fullfile(FIGoutputdir,'dFNC_putamen2SM-wSensory-relationships.pdf'),'-pdf')
%%
FF = figure;
errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),std(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),'r','linestyle','none');
hold on;
errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),std(X1_HC_SC2SMcorr_recid1corr_boot),'xk','linestyle','none');
text(mean(X1_HC_SC2SMcorr_recid1corr_boot), mean(Y_SZ_thal2SMcorr_recid1corr_boot-Y_HC_thal2SMcorr_recid1corr_boot), lab)

errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),'m','linestyle','none');
errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(X1_HC_SC2SMcorr_recid1corr_boot),'xk','linestyle','none');
text(mean(X1_HC_SC2SMcorr_recid1corr_boot), mean(Y_SZ_pput2SMcorr_recid1corr_boot-Y_HC_pput2SMcorr_recid1corr_boot), lab)
plot([-0.12 0],[0 0],'k--')
xlabel('HC:subcortical sensory antagonism','FontSize',14)
%ylabel('SZ - HC:thalamus to sensory connectivity','FontSize',14)
set(gca,'FontSize',12)
set(FF,'color',[1 1 1],'Name','dFNC_thalput2SM-subcortex2SM-relationships');
export_fig(fullfile(FIGoutputdir,'dFNC_thalput2SM-subcortex2SM-relationships.pdf'),'-pdf')

% FF1 = figure;errorbar(mean(X2_HC_wSMcorr_recid1corr_boot),mean(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),std(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),'xr','linestyle','none');
% hold on;errorbar(mean(X2_HC_wSMcorr_recid1corr_boot),mean(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),std(X2_HC_wSMcorr_recid1corr_boot),'xk','linestyle','none');
% text(mean(X2_HC_wSMcorr_recid1corr_boot), mean(Y_SZ_thal2SMcorr_recid1corr_boot-Y_HC_thal2SMcorr_recid1corr_boot), lab)
% errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),'xr','linestyle','none');
% errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(X1_HC_SC2SMcorr_recid1corr_boot),'xm','linestyle','none');
% text(mean(X1_HC_SC2SMcorr_recid1corr_boot), mean(Y_SZ_pput2SMcorr_recid1corr_boot-Y_HC_pput2SMcorr_recid1corr_boot), lab)
% xlabel('HC:subcortical sensory antagonism','FontSize',14)
% %ylabel('SZ - HC:thalamus to sensory connectivity','FontSize',14)
% set(gca,'FontSize',12)
% set(FF1,'color',[1 1 1]);
% plot([-0.15 0.25],[0  0],'k--')
% export_fig(fullfile(FIGoutputdir,'dFNC_thalput2SM-wSensory-relationships.pdf'),'-pdf')
% 
% %FF2 = figure;errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),'xr','linestyle','none');
% hold on;errorbar(mean(X1_HC_SC2SMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(X1_HC_SC2SMcorr_recid1corr_boot),'xk','linestyle','none');
% text(mean(X1_HC_SC2SMcorr_recid1corr_boot), mean(Y_SZ_pput2SMcorr_recid1corr_boot-Y_HC_pput2SMcorr_recid1corr_boot), lab)
...
FF3= figure;errorbar(mean(X2_HC_wSMcorr_recid1corr_boot),mean(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),std(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),'r','linestyle','none');
hold on;errorbar(mean(X2_HC_wSMcorr_recid1corr_boot),mean(Y_SZ_thal2SMcorr_recid1corr_boot - Y_HC_thal2SMcorr_recid1corr_boot),std(X2_HC_wSMcorr_recid1corr_boot),'xk','linestyle','none');
text(mean(X2_HC_wSMcorr_recid1corr_boot), mean(Y_SZ_thal2SMcorr_recid1corr_boot-Y_HC_thal2SMcorr_recid1corr_boot), lab)
errorbar(mean(X2_HC_wSMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),'m','linestyle','none');
errorbar(mean(X2_HC_wSMcorr_recid1corr_boot),mean(Y_SZ_pput2SMcorr_recid1corr_boot - Y_HC_pput2SMcorr_recid1corr_boot),std(X2_HC_wSMcorr_recid1corr_boot),'xk','linestyle','none');
text(mean(X2_HC_wSMcorr_recid1corr_boot), mean(Y_SZ_pput2SMcorr_recid1corr_boot-Y_HC_pput2SMcorr_recid1corr_boot), lab)
xlabel('HC:mean within sensory coherence','FontSize',14)
%plot([0.1 0.24],[0  0],'k--')
plot([0.06 0.24],[0  0],'k--')
%ylabel('SZ - HC:putamen to sensory connectivity','FontSize',14)
set(gca,'FontSize',12)
set(FF3,'color',[1 1 1]);
export_fig(fullfile(FIGoutputdir,'dFNC_thalput2SM-wSensory-relationships.pdf'),'-pdf')
...
for ii = 1:k2, 
    lenH = length(thal_to_sm_corr_HC_allstates{matched_citystate(ii)});
    lenS = length(thal_to_sm_corr_SZ_allstates{matched_citystate(ii)});
    [rr(ii) pp(ii)] = corr([thal_to_sm_corr_HC_allstates{matched_citystate(ii)};thal_to_sm_corr_SZ_allstates{matched_citystate(ii)}],[pput_to_sm_corr_HC_allstates{matched_citystate(ii)};pput_to_sm_corr_SZ_allstates{matched_citystate(ii)}]);
 %   [rr2(ii) pp2(ii)] = corr([thal_to_sm_corr_HC_allstates{matchOrd(ii)};thal_to_sm_corr_SZ_allstates{matchOrd(ii)}],[pput_to_sm_corr_HC_allstates{matchOrd(ii)};pput_to_sm_corr_SZ_allstates{matchOrd(ii)}]);

    [rrHC(ii) ppHC(ii)] = corr([thal_to_sm_corr_HC_allstates{matched_citystate(ii)}],[pput_to_sm_corr_HC_allstates{matched_citystate(ii)}]);
    [rrSZ(ii) ppSZ(ii)] = corr([thal_to_sm_corr_SZ_allstates{matched_citystate(ii)}],[pput_to_sm_corr_SZ_allstates{matched_citystate(ii)}]);
    y = [thal_to_sm_corr_HC_allstates{matched_citystate(ii)};thal_to_sm_corr_SZ_allstates{matched_citystate(ii)}];
    x1 =[pput_to_sm_corr_HC_allstates{matched_citystate(ii)};pput_to_sm_corr_SZ_allstates{matched_citystate(ii)}];
    x1 = x1 - mean(x1);
    x2 = [zeros(lenH,1);ones(lenS,1)];
    rst{ii} = regstats(y,[x1 x2 x1.*x2]);
end
F = figure;
set(F,'Color',[1 1 1],'Name','ThalamusHyperFC_vs_PutamenHypoFC_byState','Position',[560         338        1079         606])
for ii = 1:k2, 
    
    subplot(1,5,matchOrd(ii));
    scatter([thal_to_sm_corr_HC_allstates{(ii)};thal_to_sm_corr_SZ_allstates{(ii)}],[pput_to_sm_corr_HC_allstates{(ii)};pput_to_sm_corr_SZ_allstates{(ii)}]);hold on
    lsline
    scatter([thal_to_sm_corr_HC_allstates{(ii)}],[pput_to_sm_corr_HC_allstates{(ii)}],'ko');
    scatter([thal_to_sm_corr_SZ_allstates{(ii)}],[pput_to_sm_corr_SZ_allstates{(ii)}],'ro') ;
    str = sprintf('r = %0.2g,p = %0.2g',rr(matchOrd(ii)),pp(matchOrd(ii)));
    text(-0.4,-0.4,str)
    title(['State ' num2str(matchOrd(ii))])
...
F3 = figure;set(F3,'color',[1 1 1],'Name','scatter_SC2SMcorr_wSMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
F4 = figure;set(F4,'color',[1 1 1],'Name','scatter_wSCcorr_wSMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
F5 = figure;set(F5,'color',[1 1 1],'Name','scatter_THAL2SMcorr_wSMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
F3 = figure;set(F3,'color',[1 1 1],'Name','scatter_thal2SM_wSMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
F4 = figure;set(F4,'color',[1 1 1],'Name','scatter_thal2SM_SC2SMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
F13 = figure;set(F13,'color',[1 1 1],'Name','scatter_pput2SM_wSMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
F14 = figure;set(F14,'color',[1 1 1],'Name','scatter_pput2SM_SC2SMcorr_k5_recid1corr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
...
k2 = 5;
load(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'IDXallone','aINDallone', 'Callone','SUMDallone','Dallone');
aINDallone_SZ = aINDallone(SZid,:);
aINDallone_HC = aINDallone(HCid,:);
...
for rsn = 1:5%[1 2 3 4 5 7 16  23 24 29 30 47]
...
%     set(F,'Name',[L{RSN_I(rsn)}  'Specra_byStateGroup_city'])
   %%
    t_obs_hc = zeros(1,k2);p_obs_hc = t_obs_hc;t_rb_obs_hc = t_obs_hc;
    p_obs_sz = t_obs_hc;t_obs_sz = t_obs_hc;p_rb_obs_hc = t_obs_hc;
    b_obs_hc = t_obs_hc;b_obs_sz = t_obs_hc;b_rb_obs_hc = t_obs_hc;
    t_obs_hc_goodind = zeros(1,k2);p_obs_hc_goodind = t_obs_hc;
    p_obs_sz_goodind = t_obs_hc;t_obs_sz_goodind = t_obs_hc;t_rb_obs_sz = t_obs_hc;
    b_obs_hc_goodind = t_obs_hc;b_obs_sz_goodind = t_obs_hc;p_rb_obs_sz = t_obs_hc;b_rb_obs_sz = t_obs_hc;
    t_obs_diff = t_obs_hc;p_obs_diff = p_obs_hc;t_rb_obs_diff = t_obs_hc;
    t_obs_diff_goodind = t_obs_hc;p_obs_diff_goodind = p_obs_hc;
    b_obs_diff = t_obs_hc;b_obs_diff_goodind = p_obs_hc;p_rb_obs_diff = t_obs_hc;b_rb_obs_diff = t_obs_hc;
  %  b_boot_state_hc = zeros(k2,1000);b_boot_state_sz = zeros(k2,1000);
   %%
   if rsn == 3
      %  FF = figure;set(FF,'Name',['corr_a'  L{RSN_I(rsn)} 'toSensoryICN_vs_a' L{RSN_I(rsn)} 'LFamp_state_corr_resid1model'],'Color',[1 1 1],'Position',mPOS);
        FF1 = figure;set(FF1,'Name',['rbcorr_a'  L{RSN_I(rsn)} 'toSensoryICN_vs_a' L{RSN_I(rsn)} 'LFamp_state_corr_resid1model'],'Color',[1 1 1],'Position',mPOS);
   else
     %  FF = figure;set(FF,'Name',['corr_'  L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_state_corr_resid1model'],'Color',[1 1 1],'Position',mPOS);
       FF1 = figure;set(FF1,'Name',['rbcorr_'  L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_state_corr_resid1model'],'Color',[1 1 1],'Position',mPOS);
   end
...
%       set(gca,'FontSize',14)
%       export_fig(fullfile(FIGoutputdir, [get(Fc, 'Name') '_allwin_norm_corr_resid1model.pdf']), Fc);
%       close(Fc)
...
%      figure;imagesc(-log10(squeeze(pst)).*sign(squeeze(tstst.tstat)));
  ...
  %   disp([tsts.tstat tstr.tstat])
   %  disp([ps pr]) 
     ...
     [rbbhc rbstathc] = robustfit(spectHC_lf_sum - mean(spectHC_lf_sum),tempmat_thalrhc_mean);  
     [rbbsz rbstatsz] = robustfit(spectSZ_lf_sum - mean(spectSZ_lf_sum),tempmat_thalrsz_mean);
    ...
    [rbbdiff  rbstatdiff] = robustfit(XXX,[tempmat_thalrhc_mean;tempmat_thalrsz_mean]); % diag * lfpower interaction
...
%     title(['state ' num2str(srtind(state))])
    ...
    subplot(1,k2,srtind(state));
    scatter(spectHC_lf_sum,tempmat_thalrhc_mean,'k');hold on;
    plot(spectHC_lf_sum,spectHC_lf_sum*rbbhc(2),'k')
    
    scatter(spectSZ_lf_sum,tempmat_thalrsz_mean,'r');
    plot(spectSZ_lf_sum,spectSZ_lf_sum*rbbsz(2),'r')
    %scatter(spectHC_lf_sum(inti1),tempmat_thalrhc_mean(inti1),'c','filled');
    %scatter(spectSZ_lf_sum(inti2),tempmat_thalrsz_mean(inti2),'m','filled');
    title(['state ' num2str(srtind(state))])
...
%     
     ...
     t_rb_obs_hc(state) = rbstathc.t(end);p_rb_obs_hc(state) = rbstathc.p(end);
     t_rb_obs_sz(state) = rbstatsz.t(end);p_rb_obs_sz(state) = rbstatsz.p(end);
     b_rb_obs_hc(state) = rbbhc(end);b_rb_obs_sz(state) = rbbsz(end);
     t_rb_obs_diff(state) = rbstatdiff.t(end);p_rb_obs_diff(state) = rbstatdiff.p(end);
     b_rb_obs_diff(state) = rbbdiff(end);
...
%     end
  ...
  % close(FF)
   export_fig(fullfile(FIGoutputdir,[get(FF1,'Name') '.pdf']),'-pdf',FF1)
    %%
%     if rsn==3
%         save(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_a' L{RSN_I(rsn)} 'LFamp_resid1model_corr_cband'],    'p_obs_*', 't_obs_*', 'b_obs_*','t_rb*','p_rb*','b_rb*');
%     else
%         save(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_resid1model_corr_cband'],    'p_obs_*', 't_obs_*', 'b_obs_*','t_rb*','p_rb*','b_rb*');
%     end
end
%save corr_thaltoSensoryICN_vs_thalLFamp  b_boot_state_*  p_obs_* t_obs_* b_obs_*
%%
p_rb_obs_hc_all = ones(25,1);
p_rb_obs_sz_all = ones(25,1);
p_rb_obs_diff_all = ones(25,1);
p_obs_hc_all = ones(25,1);
p_obs_sz_all = ones(25,1);
p_obs_diff_all = ones(25,1);
b_rb_obs_hc_all = ones(25,1);
b_rb_obs_sz_all = ones(25,1);
b_rb_obs_diff_all = ones(25,1);
b_obs_hc_all = ones(25,1);
b_obs_sz_all = ones(25,1);
b_obs_diff_all = ones(25,1);
t_rb_obs_hc_all = ones(25,1);
t_rb_obs_sz_all = ones(25,1);
t_rb_obs_diff_all = ones(25,1);
t_obs_hc_all = ones(25,1);
t_obs_sz_all = ones(25,1);
t_obs_diff_all = ones(25,1);
for rsn = 1:5%[1:5 ]%7 16  23 24 29 30 47]
    %rsn = 30;
    if rsn==3
        cRSN = load(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_a' L{RSN_I(rsn)} 'LFamp_resid1model_corr_cband']);
        %F = figure;set(F,'Color',[1 1 1],'Name',['beta_spect_a'  L{RSN_I(rsn)}  '_vs_ra'  L{RSN_I(rsn)} '_allone_corr_resid1model' ]);
        F1 = figure;set(F1,'Color',[1 1 1],'Name',['rb_beta_spect_a'  L{RSN_I(rsn)}  '_vs_ra'  L{RSN_I(rsn)} '_allone_corr_resid1model' ]);
    else
        cRSN = load(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_resid1model_corr_cband']);
        %F = figure;set(F,'Color',[1 1 1],'Name',['beta_spect_'  L{RSN_I(rsn)}  '_vs_r'  L{RSN_I(rsn)} '_allone_corr_resid1model' ]);
        F1 = figure;set(F1,'Color',[1 1 1],'Name',['rb_beta_spect_'  L{RSN_I(rsn)}  '_vs_r'  L{RSN_I(rsn)} '_allone_corr_resid1model' ]);
    end
    ...
    [cRSN.t_rb_obs_hc(matchOrd);cRSN.t_rb_obs_sz(matchOrd);cRSN.t_rb_obs_diff(matchOrd)]
    disp(['-----------------------------------------------------------'])
    [cRSN.p_rb_obs_hc(matchOrd);cRSN.p_rb_obs_sz(matchOrd);cRSN.p_rb_obs_diff(matchOrd)]
    disp(['-----------------------------------------------------------'])
    [cRSN.b_rb_obs_hc(matchOrd);cRSN.b_rb_obs_sz(matchOrd);cRSN.b_obs_diff(matchOrd)]
    disp(['%%%%%%%---------------------------------------------------------%%%%%%%'])
%     
%     
...
%      plot(1:5,[0 0 0 0 0],'k--')
     ...
     errorbar(1:5,cRSN.b_rb_obs_hc(matchOrd),cRSN.b_rb_obs_hc(matchOrd)./cRSN.t_rb_obs_hc(matchOrd),'k');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
     hold on;errorbar(1:5,cRSN.b_rb_obs_sz(matchOrd),cRSN.b_rb_obs_sz(matchOrd)./cRSN.t_rb_obs_sz(matchOrd),'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
     plot(1:5,[0 0 0 0 0],'k--')
     
     p_obs_hc_all((rsn-1)*k2+1:rsn*k2) = cRSN.p_obs_hc(matchOrd);
     p_obs_sz_all((rsn-1)*k2+1:rsn*k2) = cRSN.p_obs_sz(matchOrd);
     p_obs_diff_all((rsn-1)*k2+1:rsn*k2) = cRSN.p_obs_diff(matchOrd);
     
     p_rb_obs_hc_all((rsn-1)*k2+1:rsn*k2) = cRSN.p_rb_obs_hc(matchOrd);
     p_rb_obs_sz_all((rsn-1)*k2+1:rsn*k2) = cRSN.p_rb_obs_sz(matchOrd);
     p_rb_obs_diff_all((rsn-1)*k2+1:rsn*k2) = cRSN.p_rb_obs_diff(matchOrd);

     b_obs_hc_all((rsn-1)*k2+1:rsn*k2) = cRSN.b_obs_hc(matchOrd);
     b_obs_sz_all((rsn-1)*k2+1:rsn*k2) = cRSN.b_obs_sz(matchOrd);
     b_obs_diff_all((rsn-1)*k2+1:rsn*k2) = cRSN.b_obs_diff(matchOrd);
     
     b_rb_obs_hc_all((rsn-1)*k2+1:rsn*k2) = cRSN.b_rb_obs_hc(matchOrd);
     b_rb_obs_sz_all((rsn-1)*k2+1:rsn*k2) = cRSN.b_rb_obs_sz(matchOrd);
     b_rb_obs_diff_all((rsn-1)*k2+1:rsn*k2) = cRSN.b_rb_obs_diff(matchOrd);
     
     t_obs_hc_all((rsn-1)*k2+1:rsn*k2) = cRSN.t_obs_hc(matchOrd);
     t_obs_sz_all((rsn-1)*k2+1:rsn*k2) = cRSN.t_obs_sz(matchOrd);
     t_obs_diff_all((rsn-1)*k2+1:rsn*k2) = cRSN.t_obs_diff(matchOrd);
     
     t_rb_obs_hc_all((rsn-1)*k2+1:rsn*k2) = cRSN.t_rb_obs_hc(matchOrd);
     t_rb_obs_sz_all((rsn-1)*k2+1:rsn*k2) = cRSN.t_rb_obs_sz(matchOrd);
     t_rb_obs_diff_all((rsn-1)*k2+1:rsn*k2) = cRSN.t_rb_obs_diff(matchOrd);
%      export_fig(fullfile(FIGoutputdir,[get(F1,'Name') '.pdf']),'-pdf',F1)
%     export_fig(fullfile(FIGoutputdir,[get(F,'Name') '.pdf']),'-pdf',F)
%     
%     F1 = figure;set(F1,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r' L{RSN_I(rsn)}   '_goodind_allone_corr_resid1model']);
...
load grp_id_cell
k2 = 5;
all1_k5 = load(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']));
aFR = zeros(M,k2);
aTM = zeros(M,k2,k2);
aMDT = zeros(M,k2);
aNT = zeros(M,1);
...
set(I3, 'Name', 'Average_STATESTATS_HCSZnewcmap_corr_resid1model')
...
for site = 1:7
  ...
  %  subplot(4,2,site)
    ...
    set(gcf, 'Name', ['Average_STATESTATS_HCSZ_site' num2str(site) 'newcmap_corr_resid1model'])
    C = colorbar;
    ylm = get(C,'YLim');
