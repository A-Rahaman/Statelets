...
load /export/mialab/users/eswar/fbirn_p3/scripts/orderedRSN_by_modularity
...
load /export/mialab/users/eswar/fbirn_p3/scripts/orderedRSN_by_modularity
load /export/mialab/users/eswar/fbirn_p3/scripts/DEMO_ICA_fBIRNp3_New
...
[pth fnm ]= fileparts(inname);
save(fullfile(pth,[fnm '_residualized_1mancova']),'rFdyn')
%%
[pth fnm ]= fileparts(inname);
...
% 
 save(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew_corr_resid1model']), 'SPflat', 'IDXp','Cp');
% load(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat', 'IDXp','Cp');
G = figure('Color', 'w', 'Name', ['FNC_group_clusters_k' num2str(k2) 'scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
for ii = 1:k2, 
    subplot(1,k2,ii);
...
%     
   ...
   %  save(fullfile(fileparts(inname), ['FNC_all_states_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'aIND')
    ...
    for ii = 1:k2,
        subplot(1,k2,ii);
        md1 = vec2mat(median(Fdynflat(IDXallone == ii ,:)),1);
        imagesc(md1(mnOrd_final_ord,mnOrd_final_ord), [-.5,.5]); axis square;
        set(gca, 'XTick', [], 'YTick', [])
        title(sprintf('%d (%d%%)', sum(IDXallone == ii),  round(100*sum(IDXallone == ii)/length(IDXallone))));
    end
    IM = export_fig(fullfile(FIGoutputdir, [get(G, 'Name') '.pdf']), G);
...
%    save(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model']), 'IDXallone','aINDallone', 'Callone','SUMDallone','Dallone')
%     
...
%     G = figure('Color', 'w', 'Name', ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
%     for ii = 1:k2,
%         subplot(2,k2,ii);
...
%     IM = export_fig(fullfile(FIGoutputdir, [get(G, 'Name') '.pdf']), G);
    ...
    H = figure('Color', 'w', 'Name', ['FNC_clusters_allone_occurrenceHCSZ_k' num2str(k2) '_scrubbed_cnew_corr_resid1model'], 'Position', CPOS);
    for ii = 1:k2,
        subplot(2,k2,ii);
        for bb = 1:100
            pickME1 = ceil(nHC*rand(nHC,1));
...
save(fullfile(fileparts(inname), ['mn_mdn_FNC_clusters_alloneByDiag_kall_scrubbed_cnew_corr_resid1model']), 'mean_Kwin_allHC', 'mean_Kwin_allSZ' , 'mdn_Kwin_allHC', 'mdn_Kwin_allSZ','nS_Kwin_allHC','nS_Kwin_allSZ');
%% stats
...
for k2 = 3:8
    ...
    G1 = figure('Color', 'w', 'Name', ['FNC_clustersMNGroupDiff_alloneSubj_k' num2str(k2) '_scrubbed_cnewRev_corr_resid1model'], 'Position', CPOSn);
    for jj = 1:k2
  ...
  %  close(G1)
    %%
    G2 = figure('Color', 'w', 'Name', ['FNC_clustersMDNGroupDiff_alloneSubj_k' num2str(k2) '_scrubbed_cnewRev_corr_resid1model'], 'Position', CPOSn);
    for jj = 1:k2
...
save tSNE_subjmediandata_k5 aconcat labels_aconcat ydata_tSNEd_edist_aconcat ydata_tSNEd_rdist_aconcat rdist_aconcat edist_aconcat
%%
FPOS = [635    66   965   849];
clr5 = 'rgbck';
...
figure('color',[1  1  1])
for k = 1:k2,
  subplot(k2,2,2*(k-1)+1);hist(aFR(HCid(find(aFR(HCid,k)>0)),k));
  if k==1, title('HC'); end
  if k == k2,xlabel('fraction in state');ylabel('counts'); end;
  subplot(k2,2,2*(k-1)+2);hist(aFR(SZid(find(aFR(SZid,k)>0)),k))
  if k==1, title('SZ'); end
end
export_fig(fullfile(FIGoutputdir,'fraction_dwell_corr_k5.pdf'),'-pdf',gcf)
%%
for site = 1:7
