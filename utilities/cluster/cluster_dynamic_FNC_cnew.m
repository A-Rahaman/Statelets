...
% 
% save(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat', 'IDXp','Cp');
 load(fullfile(fileparts(inname), ['FNC_group_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat', 'IDXp','Cp');
G = figure('Color', 'w', 'Name', ['FNC_group_clusters_k' num2str(k2) 'scrubbed_cnew'], 'Position', CPOS);
for ii = 1:k2, 
    subplot(1,k2,ii);
...
SPflat2 = cell2mat(SP(sublist(floor(M/2)+1:end)));
 ...
 save(fullfile(fileparts(inname), ['FNC_half1_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat1', 'IDXp1','Cp1', 'sublist');
%load(fullfile(fileparts(inname), ['FNC_half1_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat1', 'IDXp1','Cp1', 'sublist');

[IDXp2, Cp2, SUMDp2, Dp2] = kmeans(SPflat2, k2, 'distance', dmethod, 'Replicates', 1, 'MaxIter', 150, 'Display', 'iter', 'empty', 'drop', 'Start', Cp);

save(fullfile(fileparts(inname), ['FNC_half2_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat2', 'IDXp2','Cp2', 'sublist');
%load(fullfile(fileparts(inname), ['FNC_half2_clusters_k2_' num2str(k2) 'scrubbed_cnew']), 'SPflat2', 'IDXp2','Cp2', 'sublist');


G = figure('Color', 'w', 'Name', ['FNC_halfgroup_clusters_k' num2str(k2) 'scrubbed_cnew'], 'Position', CPOS);
for ii = 1:k2,
    subplot(2,k2,ii);
...
for k2 = 2:9
   ...
   % save(fullfile(fileparts(inname), ['FNC_HC_clusters_k2_' num2str(k2) '_scrubbed_cnew']), 'SPflatHC', 'IDXp1','Cp1', 'HCid');
    
   % [IDXp2, Cp2, SUMDp2, Dp2] = kmeans(SPflatSZ, k2, 'distance', dmethod, 'Replicates', 1, 'MaxIter', 150, 'Display', 'iter', 'empty', 'drop', 'Start', Cp);

    %save(fullfile(fileparts(inname), ['FNC_SZ_clusters_k2_' num2str(k2) '_scrubbed_cnew']), 'SPflatSZ', 'IDXp2','Cp2', 'SZid');
    load(fullfile(fileparts(inname), ['FNC_HC_clusters_k2_' num2str(k2) '_scrubbed_cnew']), 'SPflatHC', 'IDXp1','Cp1', 'HCid');
    load(fullfile(fileparts(inname), ['FNC_SZ_clusters_k2_' num2str(k2) '_scrubbed_cnew']), 'SPflatSZ', 'IDXp2','Cp2', 'SZid');
    G = figure('Color', 'w', 'Name', ['FNC_DIAGgroup_clusters_k' num2str(k2) '_scrubbed_cnew'], 'Position', CPOS);
    for ii = 1:k2,
        subplot(2,k2,ii);
...
%     aIND = aIND';
%     
%     save(fullfile(fileparts(inname), ['FNC_clusters_all_k' num2str(k2) '_scrubbed_cnew']), 'IDXall', 'Call')
%     save(fullfile(fileparts(inname), ['FNC_all_states_k' num2str(k2) '_scrubbed_cnew']), 'aIND')
    load(fullfile(fileparts(inname), ['FNC_clusters_all_k' num2str(k2) '_scrubbed_cnew']), 'IDXall', 'Call')
    G = figure('Color', 'w', 'Name', ['FNC_clusters_all_k' num2str(k2) '_scrubbed_cnew'], 'Position', CPOS);
    for ii = 1:k2,
        subplot(1,k2,ii);
...
%     timeline = 0:(Nwin-1); timeline = timeline + wsize/2; timeline = timeline*TR;
%     
%     H = figure('Color', 'w', 'Name', ['FNC_clusters_occurrence_k' num2str(k2) '_scrubbed_cnew'], 'Position', CPOS);
%     for ii = 1:k2,
%         subplot(1,k2,ii);
%         for bb = 1:100
...
%     G = figure('Color', 'w', 'Name', ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew'], 'Position', CPOS);
%     for ii = 1:k2,
%         subplot(2,k2,ii);
...
%     IM = export_fig(fullfile(FIGoutputdir, [get(G, 'Name') '.pdf']), G);
    ...
    H = figure('Color', 'w', 'Name', ['FNC_clusters_allone_occurrenceHCSZ_k' num2str(k2) '_scrubbed_cnew'], 'Position', CPOS);
    for ii = 1:k2,
        subplot(2,k2,ii);
        for bb = 1:100
            pickME1 = ceil(nHC*rand(nHC,1));
...
save(fullfile(fileparts(inname), ['mn_mdn_FNC_clusters_alloneByDiag_kall_scrubbed_cnew']), 'mean_Kwin_allHC', 'mean_Kwin_allSZ' , 'mdn_Kwin_allHC', 'mdn_Kwin_allSZ','nS_Kwin_allHC','nS_Kwin_allSZ');
%% stats
load(fullfile(fileparts(inname), ['mn_mdn_FNC_clusters_alloneByDiag_kall_scrubbed_cnew']), 'mean_Kwin_allHC', 'mean_Kwin_allSZ' , 'mdn_Kwin_allHC', 'mdn_Kwin_allSZ','nS_Kwin_allHC','nS_Kwin_allSZ');
for k2 = 3:8
    mx = zeros(1,k2);mdx = mx;
    for jj = 1:k2
       [hh pp ci tst] = ttest2(mean_Kwin_allHC{k2-2,jj} ,mean_Kwin_allSZ{k2-2,jj});
    ...
    G1 = figure('Color', 'w', 'Name', ['FNC_clustersMNGroupDiff_alloneSubj_k' num2str(k2) '_scrubbed_cnewRev'], 'Position', CPOSn);
    for jj = 1:k2
  ...
  %  close(G1)
    ...
    G2 = figure('Color', 'w', 'Name', ['FNC_clustersMDNGroupDiff_allSubj_k' num2str(k2) '_scrubbed_cnewRev'], 'Position', CPOSn);
    for jj = 1:k2
...
% save(fullfile(fileparts(inname),  'dfnc_SC_SM_relationships'),  'r_SC*', 'r_THAL*', 'r_wSC*', 'p_SC*', 'p_THAL*', 'p_wSC*', '*allstates')
%%
F = figure;set(F,'color',[1 1 1],'Name','r_SC2SMcorr_wSMcorr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    ...
    boxplot([r_SC2SMcorr_wSMcorr_HC_boot(:,st);r_SC2SMcorr_wSMcorr_SZ_boot(:,st)],[ones(nBOOT,1);2*ones(nBOOT,1)],'notch','on')
    title(['state ' num2str(st)])
end

F1 = figure;set(F1,'color',[1 1 1],'Name','r_wSCcorr_wSMcorr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
    boxplot([r_wSCcorr_wSMcorr_HC_boot(:,st);r_wSCcorr_wSMcorr_SZ_boot(:,st)],[ones(nBOOT,1);2*ones(nBOOT,1)],'notch','on')
    title(['state ' num2str(st)])
end

F2 = figure;set(F2,'color',[1 1 1],'Name','r_THAL2SMcorr_wSMcorr','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
    boxplot([r_THAL2SMcorr_wSMcorr_HC_boot(:,st);r_THAL2SMcorr_wSMcorr_SZ_boot(:,st)],[ones(nBOOT,1);2*ones(nBOOT,1)],'notch','on')
    title(['state ' num2str(st)])
end
%
L1out = load(fullfile(fileparts(inname),  'dfnc_SC_SM_relationships'));
F3 = figure;set(F3,'color',[1 1 1],'Name','scatter_SC2SMcorr_wSMcorr_k5_L1','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
    plot(L1out.sc_to_sm_corr_HC_allstates{st},L1out.within_sm_corr_HC_allstates{st},'ko');hold;
    plot(L1out.sc_to_sm_corr_SZ_allstates{st},L1out.within_sm_corr_SZ_allstates{st},'ro');
    xlim([-0.4 0.2])
    title(['state ' num2str(st)])
end


F4 = figure;set(F4,'color',[1 1 1],'Name','scatter_wSCcorr_wSMcorr_k5_L1','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
    plot(L1out.within_sc_corr_HC_allstates{st},L1out.within_sm_corr_HC_allstates{st},'ko');lsline;hold;
    plot(L1out.within_sc_corr_SZ_allstates{st},L1out.within_sm_corr_SZ_allstates{st},'ro');lsline;
    title(['state ' num2str(st)])
end

F5 = figure;set(F5,'color',[1 1 1],'Name','scatter_THAL2SMcorr_wSMcorr_k5_L1','Position',[309         659        1006         206]);
for st = 1:k2,
    subplot(1,5,st);
    % scatter(ones(nBOOT,1),r_SC2SMcorr_wSMcorr_HC_boot(:,st));hold;
    % scatter(1.1*ones(nBOOT,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,st),'r');xlim([0.9 1.2]);
    plot(L1out.thal_to_sm_corr_HC_allstates{st},L1out.within_sm_corr_HC_allstates{st},'ko');lsline;hold;
    plot(L1out.thal_to_sm_corr_SZ_allstates{st},L1out.within_sm_corr_SZ_allstates{st},'ro');lsline;
    title(['state ' num2str(st)])
end


stats_SC2SMcorr_wSMcorr_st1 = mwwtest(r_SC2SMcorr_wSMcorr_HC_boot(:,1),r_SC2SMcorr_wSMcorr_SZ_boot(:,1));
stats_SC2SMcorr_wSMcorr_st2 = mwwtest(r_SC2SMcorr_wSMcorr_HC_boot(:,2),r_SC2SMcorr_wSMcorr_SZ_boot(:,2));
stats_SC2SMcorr_wSMcorr_st3 = mwwtest(r_SC2SMcorr_wSMcorr_HC_boot(:,3),r_SC2SMcorr_wSMcorr_SZ_boot(:,3));
stats_SC2SMcorr_wSMcorr_st4 = mwwtest(r_SC2SMcorr_wSMcorr_HC_boot(:,4),r_SC2SMcorr_wSMcorr_SZ_boot(:,4));
stats_SC2SMcorr_wSMcorr_st5 = mwwtest(r_SC2SMcorr_wSMcorr_HC_boot(:,5),r_SC2SMcorr_wSMcorr_SZ_boot(:,5));

stats_THAL2SMcorr_wSMcorr_st1 = mwwtest(r_THAL2SMcorr_wSMcorr_HC_boot(:,1),r_THAL2SMcorr_wSMcorr_SZ_boot(:,1));
stats_THAL2SMcorr_wSMcorr_st2 = mwwtest(r_THAL2SMcorr_wSMcorr_HC_boot(:,2),r_THAL2SMcorr_wSMcorr_SZ_boot(:,2));
stats_THAL2SMcorr_wSMcorr_st3 = mwwtest(r_THAL2SMcorr_wSMcorr_HC_boot(:,3),r_THAL2SMcorr_wSMcorr_SZ_boot(:,3));
stats_THAL2SMcorr_wSMcorr_st4 = mwwtest(r_THAL2SMcorr_wSMcorr_HC_boot(:,4),r_THAL2SMcorr_wSMcorr_SZ_boot(:,4));
stats_THAL2SMcorr_wSMcorr_st5 = mwwtest(r_THAL2SMcorr_wSMcorr_HC_boot(:,5),r_THAL2SMcorr_wSMcorr_SZ_boot(:,5));

stats_wSCcorr_wSMcorr_st1 = mwwtest(r_wSCcorr_wSMcorr_HC_boot(:,1),r_wSCcorr_wSMcorr_SZ_boot(:,1));
stats_wSCcorr_wSMcorr_st2 = mwwtest(r_wSCcorr_wSMcorr_HC_boot(:,2),r_wSCcorr_wSMcorr_SZ_boot(:,2));
stats_wSCcorr_wSMcorr_st3 = mwwtest(r_wSCcorr_wSMcorr_HC_boot(:,3),r_wSCcorr_wSMcorr_SZ_boot(:,3));
stats_wSCcorr_wSMcorr_st4 = mwwtest(r_wSCcorr_wSMcorr_HC_boot(:,4),r_wSCcorr_wSMcorr_SZ_boot(:,4));
stats_wSCcorr_wSMcorr_st5 = mwwtest(r_wSCcorr_wSMcorr_HC_boot(:,5),r_wSCcorr_wSMcorr_SZ_boot(:,5));

%%

disp([r_SC2SMcorr_wSMcorr_HC_obs(matchOrd);r_SC2SMcorr_wSMcorr_SZ_obs(matchOrd); ...
 L1out.r_SC2SMcorr_wSMcorr_HC_obs;L1out.r_SC2SMcorr_wSMcorr_SZ_obs])
disp([p_SC2SMcorr_wSMcorr_HC_obs(matchOrd);p_SC2SMcorr_wSMcorr_SZ_obs(matchOrd); ...
 L1out.p_SC2SMcorr_wSMcorr_HC_obs;L1out.p_SC2SMcorr_wSMcorr_SZ_obs])

disp([r_wSCcorr_wSMcorr_HC_obs(matchOrd);r_wSCcorr_wSMcorr_SZ_obs(matchOrd); ...
 L1out.r_wSCcorr_wSMcorr_HC_obs;L1out.r_wSCcorr_wSMcorr_SZ_obs])
disp([p_wSCcorr_wSMcorr_HC_obs(matchOrd);p_wSCcorr_wSMcorr_SZ_obs(matchOrd); ...
 L1out.p_wSCcorr_wSMcorr_HC_obs;L1out.p_wSCcorr_wSMcorr_SZ_obs])

disp([r_THAL2SMcorr_wSMcorr_HC_obs(matchOrd);r_THAL2SMcorr_wSMcorr_SZ_obs(matchOrd); ...
 L1out.r_THAL2SMcorr_wSMcorr_HC_obs;L1out.r_THAL2SMcorr_wSMcorr_SZ_obs])
disp([p_THAL2SMcorr_wSMcorr_HC_obs(matchOrd);p_THAL2SMcorr_wSMcorr_SZ_obs(matchOrd); ...
 L1out.p_THAL2SMcorr_wSMcorr_HC_obs;L1out.p_THAL2SMcorr_wSMcorr_SZ_obs])

% for st=1:5
%     plot(st,corr(sc_to_sm_ant_corr_HC_allstates{st}, within_sm_corr_HC_allstates{st}),'*')
%     plot(st,corr(sc_to_sm_ant_corr_SZ_allstates{st}, within_sm_corr_SZ_allstates{st}),'o')
% end
...
%%
for rsn = [4 5] %23%[1  5 7 16  23 24 29 47]
...
%     tempmat_thalrsz_meanortho = tempmat_thalrsz_mean - remsz;
    ...
    FF = figure;set(FF,'Name',['corr_'  L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_state' num2str(state)],'Color',[1 1 1]);
...
%     end
    ...
    save(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_mFD_Diag_L1_cband'],    'p_obs_*', 't_obs_*', 'b_obs_*');
end
...
for rsn = [4 5]%[1  5 7 16  23 24 29 30 47]
    %rsn = 30;
    cRSN = load(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_mFD_Diag_L1_cband']);
...
%     F = figure;set(F,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r'  L{RSN_I(rsn)} '_allone_L1' ]);
...
%     
%     F1 = figure;set(F1,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r' L{RSN_I(rsn)}   '_goodind_allone_L1']);
...
%% 
F = figure;set(F,'Color',[1 1 1],'Name','beta_spectThal_vs_rThal');
errorbar(1:5,b_obs_hc,b_obs_hc./t_obs_hc);xlim([0.5 5.5]); set(gca,'XTick',[1:5])
hold on;errorbar(1:5,b_obs_sz,b_obs_sz./t_obs_sz,'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
plot(1:5,[0 0 0 0 0],'k--')
export_fig(fullfile(FIGoutputdir,[get(F,'Name') '.pdf']),'-pdf',F)

F1 = figure;set(F1,'Color',[1 1 1],'Name','beta_spectThal_vs_rThal_goodind');
errorbar(1:5,b_obs_hc_goodind,b_obs_hc_goodind./t_obs_hc_goodind);xlim([0.5 5.5]); set(gca,'XTick',[1:5])
hold on;errorbar(1:5,b_obs_sz_goodind,b_obs_sz_goodind./t_obs_sz_goodind,'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
plot(1:5,[0 0 0 0 0],'k--')
export_fig(fullfile(FIGoutputdir,[get(F1,'Name') '.pdf']),'-pdf',F1)
%% 
rPCC = load('corr_PCCtoSensoryICN_vs_PCCLFamp_mFD_Diag.mat');
F = figure;set(F,'Color',[1 1 1],'Name','beta_spectPCC_vs_rPCC');
errorbar(1:5,rPCC.b_obs_hc,rPCC.b_obs_hc./t_obs_hc,'k');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
hold on;errorbar(1:5,rPCC.b_obs_sz,rPCC.b_obs_sz./t_obs_sz,'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
plot(1:5,[0 0 0 0 0],'k--')
export_fig(fullfile(FIGoutputdir,[get(F,'Name') '.pdf']),'-pdf',F)

F1 = figure;set(F1,'Color',[1 1 1],'Name','beta_spectPCC_vs_rPCC_goodind');
errorbar(1:5,rPCC.b_obs_hc_goodind,rPCC.b_obs_hc_goodind./t_obs_hc_goodind,'k');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
hold on;errorbar(1:5,rPCC.b_obs_sz_goodind,rPCC.b_obs_sz_goodind./t_obs_sz_goodind,'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
plot(1:5,[0 0 0 0 0],'k--')
export_fig(fullfile(FIGoutputdir,[get(F1,'Name') '.pdf']),'-pdf',F1)
...
rsn = 30;
rPrecu = load(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_mFD_Diag']);
F = figure;set(F,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r'  L{RSN_I(rsn)}  ]);
errorbar(1:5,rPrecu.b_obs_hc,rPrecu.b_obs_hc./t_obs_hc,'k');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
hold on;errorbar(1:5,rPrecu.b_obs_sz,rPrecu.b_obs_sz./t_obs_sz,'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
plot(1:5,[0 0 0 0 0],'k--')
export_fig(fullfile(FIGoutputdir,[get(F,'Name') '.pdf']),'-pdf',F)

F1 = figure;set(F1,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r' L{RSN_I(rsn)}   '_goodind']);
errorbar(1:5,rPrecu.b_obs_hc_goodind,rPrecu.b_obs_hc_goodind./t_obs_hc_goodind,'k');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
hold on;errorbar(1:5,rPrecu.b_obs_sz_goodind,rPrecu.b_obs_sz_goodind./t_obs_sz_goodind,'r');xlim([0.5 5.5]); set(gca,'XTick',[1:5])
plot(1:5,[0 0 0 0 0],'k--')
export_fig(fullfile(FIGoutputdir,[get(F1,'Name') '.pdf']),'-pdf',F1)
...
load grp_id_cell
k2 = 5;
all1_k5 = load(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew']));
aFR = zeros(M,k2);
aTM = zeros(M,k2,k2);
aMDT = zeros(M,k2);
aNT = zeros(M,1);
...
set(I3, 'Name', 'Average_STATESTATS_HCSZnewcmap')
...
for site = 1:7
  ...
  %  subplot(4,2,site)
    ...
    set(gcf, 'Name', ['Average_STATESTATS_HCSZ_site' num2str(site) 'newcmap'])
    C = colorbar;
    ylm = get(C,'YLim');
