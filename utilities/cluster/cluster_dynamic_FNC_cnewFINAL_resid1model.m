...
%load(fullfile(fileparts(inname), ['FNC_half1_clusters_k2_' num2str(k2) 'scrubbed_cnew_resid1model']), 'SPflat1', 'IDXp1','Cp1', 'sublist');
...
%load(fullfile(fileparts(inname), ['FNC_half2_clusters_k2_' num2str(k2) 'scrubbed_cnew_resid1model']), 'SPflat2', 'IDXp2','Cp2', 'sublist');
...
for rsn = [1 4 5 7 16  23 24 29 30 47]
...
%     set(F,'Name',[L{RSN_I(rsn)}  'Specra_byStateGroup_city'])
  ...
  %  FF = figure;set(FF,'Name',['corr_'  L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_state_resid1model_city'],'Color',[1 1 1],'Position',mPOS);
    for state = 1:5     
...
%     
    ...
    for ss = 1:length(hid), tempmat1(ss,:,:,:) = vec2mat(squeeze(tempHC(ss,:,:)));tempmat1(ss,:,:,:) = tempmat1(ss,:,mnOrd_final_ord,mnOrd_final_ord); end
    ...
    for ss = 1:length(sid), tempmat2(ss,:,:,:) = vec2mat(squeeze(tempSZ(ss,:,:)));tempmat2(ss,:,:,:) = tempmat2(ss,:,mnOrd_final_ord,mnOrd_final_ord);end
...
%       set(gca,'FontSize',14)
%       export_fig(fullfile(FIGoutputdir, [get(Fc, 'Name') '_allwin_norm_resid1model.pdf']), Fc);
%       close(Fc)
...
%     figure(FF);subplot(1,5,srtind(state));
...
%     close(FF)
    save(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_resid1model_city_cband'],    'p_obs_*', 't_obs_*', 'b_obs_*');
end
...
for rsn = [1 4  5 7 16  23 24 29 30 47]
    %rsn = 30;
    cRSN = load(['corr_' L{RSN_I(rsn)} 'toSensoryICN_vs_' L{RSN_I(rsn)} 'LFamp_resid1model_city_cband']);
    ...
    F = figure;set(F,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r'  L{RSN_I(rsn)} '_allone_city_resid1model' ]);
    ...
    close(F)
%     F1 = figure;set(F1,'Color',[1 1 1],'Name',['beta_spect'  L{RSN_I(rsn)}  '_vs_r' L{RSN_I(rsn)}   '_goodind_allone_L1_resid1model']);
...
load grp_id_cell
k2 = 5;
all1_k5 = load(fullfile(fileparts(inname), ['FNC_clusters_allone_HCSZ_k' num2str(k2) '_scrubbed_cnew_resid1model']));
aFR = zeros(M,k2);
aTM = zeros(M,k2,k2);
aMDT = zeros(M,k2);
aNT = zeros(M,1);
...
set(I3, 'Name', 'Average_STATESTATS_HCSZnewcmap_resid1model')
...
for site = 1:7
  ...
  %  subplot(4,2,site)
    ...
    set(gcf, 'Name', ['Average_STATESTATS_HCSZ_site' num2str(site) 'newcmap_resid1model'])
    C = colorbar;
    ylm = get(C,'YLim');
