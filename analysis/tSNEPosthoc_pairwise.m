%% post hoc analysis after pairwise tSNE [for both group]
...
for i = 1:314
    ...
    Ps =p;
    Ysub = load('tSNE_allpairshape_SZ.txt');%load(fullfile(dataDir,['Y_Subject_',num2str(i),'.txt']));
    X = Ysub(:,1);
    Y = Ysub(:,2);
...
save('/Users/mrahaman1/Documents/Statelet_V2/fResults/pMats.mat','pMats');
%% Find the peaks
...
clear
clc
load /Users/mrahaman1/Documents/Statelet_V2/data/EMDresults/pMats.mat
...
save('/Users/mrahaman1/Documents/Statelet_V2/fResults/Peaklocs.mat','peaklocs');
% imagesc(bpMat)
% hold on
% for fn=1:length(x)
...
clear
clc
load /Users/mrahaman1/Documents/Statelet_V2/fResults/Peaklocs.mat
dataDir = '/Users/mrahaman1/Documents/Statelet_V2/fResults/tSNEResults';
load /Users/mrahaman1/Documents/Statelet_V2/fResults/probabilityDensity.mat
for i = 1:length(peaklocs)
    i
    coord = peaklocs{i};
    Ysub = load(fullfile(dataDir,['Y_Subject_',num2str(i),'.txt']));
    P = pd{i};
...
for h =1:size(peaksIJ,2)
  %load (fullfile('/Users/mrahaman1/Documents/Statelet_V2/fResults/shapeletsSubWISE',['sub_',num2str(h,'%03.f')]));
  load (fullfile('/Users/mrahaman1/Documents/Statelet_V2/fResults/subWISE_real_shapes',['subWISE_real_shapes_',num2str(h),'.mat']));
  kk = peaksIJ{h};
  %shapes = zeros(length(kk),50);
  for pp = 1:length(kk)
      %shapes(pp,:) = allshapes_s{kk(pp)};
      %allSUBshapes(count,:) = allshapes_s{kk(pp)};
      allSUBshapes_real{count} = subwise_real_shapes{kk(pp)}; % for grabing the real length 
      count = count+1;
  end
  %subWISEshapes{h} = shapes;
end
save(fullfile(dirr,['subWISEshapes_',num2str(emd),'.mat']),'subWISEshapes');
save(fullfile(dirr,['allSUBshapes_',num2str(emd),'.mat']),'allSUBshapes');
...
for j = 1:size(allSUBshapes,1)
  x = allSUBshapes(j,:);  
  
  for k = 1:size(allSUBshapes,1)
    %if j~=k
    y = allSUBshapes(k,:);
    x = x-min(x);
    x = x/sum(x);
...
clear
clc
load /Users/mrahaman1/Documents/Statelet_V2/fResults/SZPaircounts.mat
load /Users/mrahaman1/Documents/Statelet_V2/fResults/HCPaircount.mat
...
% 6-6
SZ_with_f6 = find(SZ_i==6);
HC_with_f6 = find(HC_i==6);
f6_6       = intersect(SZ_with_f6,HC_with_f6);    
for i=1:length(f6_6)
    shapes6_6 = find(shape_to_pair==f6_6(i));
    groups   =  shape_to_subcl(shapes6_6);
    
    SZidx   = find(groups==0);
    shapes6_6_SZ = shapes6_6(SZidx);
    [~,hfreqshapes6_6_SZ_i] = max(p(shapes6_6_SZ));
    hfs6_6_SZ(i) = shapes6_6_SZ(hfreqshapes6_6_SZ_i);
    
    HCidx   = find(groups==1);
    shapes6_6_HC = shapes6_6(HCidx);
    [~,hfreqshapes6_6_HC_i] = max(p(shapes6_6_HC));
    hfs6_6_HC(i) = shapes6_6_HC(hfreqshapes6_6_HC_i);
end
...
hfs_SZ_all = [hfs6_6_SZ hfs5_5_SZ hfs4_4_SZ hfs4_5_SZ hfs5_4_SZ];
hfs_HC_all = [hfs6_6_HC hfs5_5_HC hfs4_4_HC hfs4_5_HC hfs5_4_HC];
%$$$$$$$$$$$$$$$$$$$$$$$$$ plot this
figure()
