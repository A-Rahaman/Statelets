...
clear
clc
load /home/users/mrahaman1/Statelet_V2/data/SZsubjectIdx.mat
load /home/users/mrahaman1/Statelet_V2/data/HCsubjectIdx.mat
load /home/users/mrahaman1/Statelet_V2/data/candidates_22to50.mat
co_occurences_SZ = zeros(1081,1080);
co_occurences_HC = zeros(1081,1080);
lag_HC = zeros(1081,1080);
lag_SZ = zeros(1081,1080);
...
dirr1 = '/home/users/mrahaman1/Statelet_V2/fResults/laggings';
 ...
 %for i = 1:size(SZIdx,1)
     load(fullfile('/home/users/mrahaman1/Statelet_V2/fResults/subinfo',['info_SUB_',num2str(SZIdx(i),'%03.f')]));
     ...
     for j = 1:size(Info_i,1)
        ...
        for k = 1:size(k_pairs,2)
            ...
            co_occurences_SZ(j,k) = cooc_count/length(occur_ij);         % Co-occurences betn pair j and k of sub i
            lag_SZ(j,k)           = lag/length(occur_ij); 
         end
     end
      save(fullfile(dirr,['co-occur_',num2str(SZIdx(i)),'.mat']),'co_occurences_SZ');
      save(fullfile(dirr1,['lag_',num2str(SZIdx(i)),'.mat']),'lag_SZ');
 %% HC
 else
 %for i = 1:size(CtIdx,1)
     load(fullfile('/home/users/mrahaman1/Statelet_V2/fResults/subinfo',['info_SUB_',num2str(CtIdx(i),'%03.f')]));
     Info_i  = repWavelet_Details;
     for j = 1:size(Info_i,1)
        cands_ij = candidates{1,Info_i{j,1}};                              % Candidates for pair j and subject i
        occur_ij = Info_i{j,3};                                            % Occurences for pair j of subject i 
        ...
        for k = 1:size(k_pairs,2)
            ...
            for oc_j = 1:length(occur_ij)
                for oc_k =1:length(occur_ik)
                    %overlap = length(intersect(cands_ij{occur_ij(oc_j)}, cands_ik{occur_ik(oc_k)}));
                    oPercent = (length(cands_ij{occur_ij(oc_j)})*25)/100;
                    if(length(intersect(cands_ij{occur_ij(oc_j)}, cands_ik{occur_ik(oc_k)}))>=oPercent)
                        cooc_count = cooc_count+1;                         % Co-occurence count incremented
                        sh1 = cands_ij{occur_ij(oc_j)};
            ...
            co_occurences_HC(j,k) = cooc_count/length(occur_ij);         % Co-occurences betn pair j and k of sub i
            lag_HC(j,k)           = lag/length(occur_ij); 
        end
     end
      save(fullfile(dirr,['co-occur_',num2str(CtIdx(i)),'.mat']),'co_occurences_HC');
      save(fullfile(dirr1,['lag_',num2str(CtIdx(i)),'.mat']),'lag_HC');
 end
end
